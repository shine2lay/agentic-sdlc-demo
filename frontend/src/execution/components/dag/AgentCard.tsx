...
export const AgentCard = memo(function AgentCard({ agentId }: AgentCardProps) {
  ...
  return (
    <div
      className="bg-[var(--temper-panel)] rounded px-3 py-2 cursor-pointer hover:bg-[var(--temper-panel-light)] transition-colors focus:outline-none focus:ring-2 focus:ring-[var(--temper-accent)]/50 focus:ring-offset-1 focus:ring-offset-[var(--temper-panel)]"
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-1">
        <span className="text-sm font-medium text-[var(--temper-text)] truncate">
          {agent.agent_name ?? agent.name ?? agentId}
        </span>
        {model && (
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-[var(--temper-surface)] text-[var(--temper-text-muted)] shrink-0">
            {model}
          </span>
        )}
        ...
        {agent.confidence_score != null && (
          <span
            className={cn(
              ...
              confidenceBadgeClass(agent.confidence_score),
            )}
          >
        ...
        {hasApprovalRequired && (
          <span
            className="text-[10px] px-1.5 py-0.5 rounded bg-amber-950/50 text-amber-400 border border-amber-900/50 shrink-0"
      ...
      {totalTokens > 0 && (
        <div
          className="h-1.5 w-full rounded-full bg-[var(--temper-surface)] mb-1 overflow-hidden flex"
          title={`Prompt: ${formatTokens(promptTokens)} | Completion: ${formatTokens(completionTokens)} | Total: ${formatTokens(totalTokens)}`}
        >
          <div
            className="h-full bg-[var(--color-temper-token-prompt)]"
            style={{ width: `${promptPct}%` }}
          />
          <div
            className="h-full bg-[var(--color-temper-token-completion)]"
      ...
      {/* Metrics */}
      <div className="flex items-center gap-3 text-[10px] text-[var(--temper-text-muted)]">
      ...
      {agent.output && (
        <div
          className={cn(
            'mt-1 text-[10px] text-[var(--temper-text-dim)] font-mono cursor-pointer',
