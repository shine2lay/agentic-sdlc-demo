...
import { selectStageGroups, selectDagInfo } from '../../selectors';
import { useDagElements } from '../../hooks/useDagElements';
import { computeStagePositions } from '../../dagLayout';
...
export function DagView() {
  ...
  const relayoutFromMeasurements = useCallback(() => {
    ...
    const updatedNodes = rfNodes.map((node) => {
      ...
      if (
        pos &&
        (Math.abs(node.position.x - pos.x) > 1 ||
          Math.abs(node.position.y - pos.y) > 1)
      ) {
        changed = true;
  ...
  const onContainerKeyDown = useCallback(
    (e: KeyboardEvent<HTMLDivElement>) => {
      ...
      if (e.key === 'Tab') {
        ...
        const next =
          (focusedNodeIndexRef.current + dir + stageNodes.length) % stageNodes.length;
