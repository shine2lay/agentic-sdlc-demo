...
export function SummaryBar() {
  ...
  const pipeline = useMemo(() => buildPipeline(stages), [stages]);
  ...
  return (
    <div className="flex items-center gap-4 flex-wrap bg-gray-900/50 px-4 py-2 border-b border-gray-700/60 shrink-0">
      {stats.map((s) => (
      ...
      {stageCosts.length > 0 && (() => {
        ...
        return (
          <div className="flex items-center gap-2 ml-2 border-l border-gray-700/40 pl-3">
            <span className="text-[10px] text-gray-600 shrink-0">Cost/stage:</span>
            <div className="flex items-end gap-1 h-4">
              {stageCosts.slice(0, 5).map((s) => (
                <div
                  key={s.name}
                  className="flex flex-col items-center"
                  title={`${s.name}: ${formatCost(s.cost)}`}
                >
                  <div
                    className="w-3 bg-blue-500/60 rounded-t-sm"
                    style={{ height: Math.max((s.cost / maxCost) * 16, 2) }}
                  />
                </div>
              ))}
            </div>
      ...
      {pipeline.length > 0 && (
        <div
          className="flex items-center gap-0.5 ml-2 border-l border-gray-700/40 pl-3"
