...
export function JsonViewer({ data, className }: JsonViewerProps) {
  ...
  return (
    <div className={cn('relative rounded-md bg-gray-800 border border-gray-700', className)}>
...
function CollapsibleNode({
  ...
  return (
    <span>
      <button
        onClick={onToggle}
        className="text-gray-500 hover:text-gray-300 mr-1 text-[10px]"
      >
        {expanded ? '\u25BC' : '\u25B6'}
      </button>
      ...
      {expanded ? (
        <>
          <div>{children}</div>
          <span className="text-gray-300">{bracket[1]}</span>
        </>
      ) : (
        <span>
