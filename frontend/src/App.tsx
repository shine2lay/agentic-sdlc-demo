...
import { fetchHealth, fetchRuns, submitSuggestion, fetchRun, type Run } from './api';
...
function App() {
  ...
  const handleRunClick = async (run: Run) => {
    if (!hasExecution(run)) return;
    try {
      const fullRun = await fetchRun(run.id);
      const result = fullRun.result as Record<string, unknown> | null;
      if (result?.execution) {
        setSelectedExecution(result.execution as WorkflowExecution);
      }
    } catch {
      // ignore
    }
    ...
    <div className="max-w-4xl mx-auto p-8">
      ...
      <div className="mb-8">
        ...
        <div className="flex gap-3 items-end">
          ...
          />
          <button
            className="bg-[var(--temper-accent)] text-[var(--temper-panel)] rounded-lg px-5 py-2 text-sm font-medium whitespace-nowrap hover:opacity-85 disabled:opacity-40 disabled:cursor-not-allowed transition-opacity"
          ...
          {runs.map((run) => {
            ...
            return (
              <div
                ...
                onClick={() => clickable && handleRunClick(run)}
              >
